...
export default function OnboardingStep2() {
  ...
  return (
    <div className="animate-in">
      <div className="mb-8">
        <p className="text-xs font-semibold text-primary-500 uppercase tracking-widest mb-2">Step 2 of 3</p>
        <h1 className="text-2xl font-bold text-neutral-900 mb-2">Choose your plan</h1>
        <p className="text-sm text-secondary-500">Start free and upgrade anytime. No credit card required.</p>
      </div>

      <div className="h-1 bg-neutral-200 rounded-full mb-10 overflow-hidden">
        <div className="h-full bg-primary-500 rounded-full" style={{ width: "66%" }} role="progressbar" aria-valuenow={66} aria-valuemin={0} aria-valuemax={100} aria-label="Step 2 of 3" />
      </div>
        ...
        {plans.map((plan) => (
          <button
            ...
            className={cn(
              ...
              selected === plan.id ? "border-primary-500 bg-primary-50" : "border-neutral-200 bg-white hover:border-neutral-300"
            )}
          >
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-3">
                <div className={cn("h-4 w-4 rounded-full border-2 flex items-center justify-center shrink-0", selected === plan.id ? "border-primary-500 bg-primary-500" : "border-neutral-300")} aria-hidden="true">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {plan.highlighted && <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-accent-100 text-accent-700">Popular</span>}
                  </div>
            ...
            <ul className="ml-7 grid grid-cols-2 gap-1">
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-center gap-1.5 text-xs text-secondary-600">
                  <CheckCircle className="h-3 w-3 text-success-500 shrink-0" aria-hidden="true" />
                  {feature}
                </li>
              ))}
            </ul>

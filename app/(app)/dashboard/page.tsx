...
const stats = [
  { label: "Total users",       value: "12,847", change: "+12.5%", positive: true,  icon: Users,    color: "text-primary-600", bg: "bg-primary-50" },
  ...
  { label: "Avg. response time",value: "143ms",  change: "+0.8%",  positive: true,  icon: BarChart3,color: "text-neutral-600",  bg: "bg-neutral-100"},
];
  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6 scrollbar-thin">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-neutral-900 tracking-tight">Good morning, {displayName}</h1>
          <p className="text-sm text-neutral-500 mt-1">Here&apos;s what&apos;s happening across your workspace today.</p>
        </div>
            ...
            return (
              <Card key={stat.label} padding="md" hover>
                <div className="flex items-start justify-between mb-4">
                  <span className={`h-10 w-10 rounded-xl ${stat.bg} flex items-center justify-center`}>
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2">
            <Card padding="lg">
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="text-xs text-primary-600 hover:text-primary-700 font-medium transition-colors">View all</button>
              </div>
              <ul role="list" className="space-y-4">
                {recentActivity.map((item, i) => (
                  <li key={item.id} className={`flex items-start gap-4 ${i < recentActivity.length - 1 ? "pb-4 border-b border-neutral-100" : ""}`}>
                    <span className="h-8 w-8 rounded-full bg-gradient-subtle flex items-center justify-center text-xs font-semibold text-primary-700 shrink-0 mt-0.5" aria-hidden="true">
                      {item.user.charAt(0)}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-neutral-700">{item.description}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-neutral-400">{item.user} · {formatRelativeTime(item.timestamp)}</span>
                        <Badge
                          variant={item.status === "success" ? "primary" : item.status === "warning" ? "warning" : item.status === "error" ? "error" : "default"}
                          size="sm"
                        >
                          {item.type}
                        </Badge>
          ...
          <div className="space-y-4">
            <Card padding="lg">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { label: "Invite teammate", icon: Users     },
                  { label: "View analytics",  icon: BarChart3 },
                ].map(({ label, icon: Icon }) => (
                  <button
                    key={label}
                    type="button"
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-neutral-700 hover:bg-neutral-50 hover:text-neutral-900 transition-colors text-left border border-neutral-200 hover:border-neutral-300"
                  >
            ...
            <div className="relative rounded-2xl bg-gradient-primary p-5 overflow-hidden">
              ...
              <div className="relative text-white">
                ...
                <div className="h-1.5 rounded-full bg-primary-400/50 overflow-hidden">
                  <div
